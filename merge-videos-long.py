...
def main():
    ...
    # Step 1: stream-copy concat into a temporary file.
    # This is fast but may produce slight timestamp rounding errors across
    # 100k+ frames (visible as avg_frame_rate ≈ 24.997 instead of exactly 25)
    # and a non-zero start_pts, both of which cause YouTube to classify the
    # video as SD even though it is 1920×1080.
    run(['ffmpeg', '-y',
         '-f', 'concat', '-safe', '0', '-i', 'concat_list.txt',
         '-c', 'copy',
         'final_concat_raw.mp4'])

    # Step 2: normalize pass — re-encode with:
    #   • fps filter: locks every frame to exactly 25 fps (fixes VFR)
    #   • -profile:v high: YouTube expects High profile for 1080p HD classification
    #   • -level 4.0: correct for 1920×1080 @ 25fps
    #   • -crf 18: near-lossless quality (visually identical to source)
    #   • -preset slow: better compression efficiency (use 'fast' to go faster)
    #   • -vsync cfr: forces constant frame rate output
    #   • -c:a aac + aresample=async=1: re-encodes audio with async drift correction
    #     so audio duration exactly matches video duration (fixes the 17ms mismatch
    #     that occurs when video is re-encoded but audio is stream-copied)
    #   • -movflags +faststart: moov atom at start for streaming
    # This pass re-encodes both streams (~10–20 min on CPU for 80 min video).
    print("  Re-encoding for YouTube HD compatibility (this may take a while) ...")
    run(['ffmpeg', '-y',
         '-i', 'final_concat_raw.mp4',
         '-vf', f'fps={fps_s}',           # lock to exact constant frame rate
         '-vsync', 'cfr',                 # constant frame rate output
         '-c:v', 'libx264',
         '-profile:v', 'high',            # High profile → YouTube classifies as HD
         '-level:v', '4.0',              # correct for 1920×1080 @ 25fps
         '-pix_fmt', 'yuv420p',
         '-crf', '18',                    # near-lossless; raise to 23 to encode faster
         '-preset', 'slow',              # better quality/size ratio
         '-c:a', 'aac',                   # re-encode audio so its duration tracks video
         '-af', 'aresample=async=1',      # correct any A/V drift sample-by-sample
         '-ar', '48000',                  # preserve original sample rate
         '-ac', '2',                      # stereo
         '-b:a', '128k',                  # preserve original audio bitrate
         '-movflags', '+faststart',       # streaming-optimised mp4
         'final_output.mp4'])
    # GPU acceleration alternative (uncomment if you have NVIDIA GPU):
    # run(['ffmpeg', '-y',
    #      '-i', 'final_concat_raw.mp4',
    #      '-vf', f'fps={fps_s}',
    #      '-vsync', 'cfr',
    ...
    #      '-profile:v', 'high',
    #      '-level:v', '4.0',
    #      '-rc', 'vbr', '-cq', '18',     # quality equivalent to -crf 18
